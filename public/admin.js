...
$('settings').onsubmit = async event => {
  ...
  try {
    await api('/api/admin/settings', { method: 'POST', body: JSON.stringify({ deepseekKey: $('apiKey').value, model: $('model').value, websiteUrl: $('website').value, crawlEnabled: $('crawlEnabled').checked, origins: $('origins').value, botTitle: $('title').value, botColor: $('color').value, extraUrls: $('extraUrls').value, customText: $('customText').value, botInstructions: $('instructions').value, welcomeMessage: $('welcome').value, commands: $('commands').value, zaloUrl: $('zaloUrl').value, messengerUrl: $('messengerUrl').value, showZalo: $('showZalo').checked, showMessenger: $('showMessenger').checked, showAssistant: $('showAssistant').checked }) });
