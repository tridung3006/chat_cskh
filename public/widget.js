(async () => {
  ...
  let menuTimer;
  const closeMenu = () => { clearTimeout(menuTimer); menuTimer = undefined; menu.classList.remove('show'); launcher.style.visibility = 'visible'; launcher.style.opacity = '1'; launcher.style.transform = 'scale(1)'; launcher.setAttribute('aria-expanded', 'false'); };
  launcher.onclick = () => { clearTimeout(menuTimer); menu.classList.add('show'); launcher.style.visibility = 'hidden'; launcher.style.opacity = '0'; launcher.style.transform = 'scale(.85)'; launcher.setAttribute('aria-expanded', 'true'); menuTimer = setTimeout(closeMenu, 5000); };
